...
const btnExpense = document.querySelector('#new-servicio');
...
async function getContent(){
  const html = await fetch('http://localhost/feralapp/servicios/create').then(res => res.text());
  return html;
}
...
google.charts.load('current', {'packages':['bar']});
      ...
      async function drawChart() {
        const http = await fetch('http://localhost/feralapp/servicios/getServiciosJSON')
        .then(json => json.json())
        .then(res => res);
        ...
        var data = google.visualization.arrayToDataTable(servicios);
